...
async function loadReelThumbnails() {
  ...
  for (const reelId of reelLinks) {
    try {
      const reelItem = document.createElement('div');
      reelItem.className = 'reel-item';
      
      // Instagram's thumbnail URL pattern
      const thumbnailUrl = `https://www.instagram.com/p/${reelId}/media/?size=m`;
      
      reelItem.innerHTML = `
        <a href="https://www.instagram.com/reel/${reelId}/" target="_blank">
          <img src="${thumbnailUrl}" alt="Reel thumbnail" onerror="this.src='fallback-image.jpg'">
          <div class="reel-overlay">▶️</div>
        </a>
      `;
      reelGrid.appendChild(reelItem);
